...
//        if(EmployeeInformation)
    ...
    private void CalculateSalaryActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_CalculateSalaryActionPerformed
        ...
        //Sample Calculations
        double dailyRate = BasicSalary / 20.0; //  no. of working days
        double grossIncome = dailyRate * DaysWorked + (OvertimeHours * (dailyRate / 8) * 1.25); //Overtime Rate ex: 1.25
        double benefits = grossIncome * 0.10; /// example: 10% benefits
        double deductions = grossIncome * 0.054; 
        double netIncome = grossIncome + benefits - deductions; 
        
        
        
       // Display the summary message dialog
        //Insert here a showmessagedialog using these below (Task for Allysa)
         JOptionPane.showMessageDialog(this, "Summary\n"
                                        + "Gross Income: P" + String.format ("%.2f", grossIncome) + "\n"
                                        + "Benefits: P" + String.format ("%.2f", benefits) + "\n"
                                        + "Deductions: P" + String.format ("%.2f", deductions) + "\n"
                                        + "Net Income: P" + String.format ("%.2f", netIncome)
         );
